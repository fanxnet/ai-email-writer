...
// Build mail start regex: auto match name + optional space + : or ：
const starterKeywords = THREAD_BLOCK_NAMES
    .map(s => `${escapeRegExp(s)}\\s*[:：]`)
    .join('|');
const mailStartRx = new RegExp(`^[\\s\\u00A0]*(${starterKeywords})`, 'i');

// Build extra header regex
const extraHeaderRxItems = HEADER_NAMES_TO_REMOVE
...
const removableHeaderNames = new Set(
    HEADER_NAMES_TO_REMOVE.map(s => s.toLowerCase())
);
...
function getHeaderSpan(
    ...
    if (index + 1 < lines.length) {
        ...
        // Header name split inside the word / phrase:
        // Assun + to: -> Assunto:
        // Envia + do el: -> Enviado el:
        // Gesen + det am: -> Gesendet am:
...
function cleanOneMailBlock(
    ...
    let inFoldedHeaderValue = false; // RFC822 折叠续行标记
    ...
    for (let i = 0; i < rawLines.length; i++) {
        ...
        if (fromFound && inHeader) {
            ...
            if (headerSpan > 0) {
                ...
                inFoldedHeaderValue = true; // 开启续行捕获
                i += headerSpan - 1;
                continue;
            }

            // Folded continuation of a removed header (old logic for word-split)
            if (
            ...
            // New mail block inside header region
            if (
                isMailStartLine(line) &&
